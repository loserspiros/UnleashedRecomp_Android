...
static constexpr double MILES_ICON_ANIMATION_TIME =
    SCANLINES_ANIMATION_TIME + 10.0;
static constexpr double MILES_ICON_ANIMATION_DURATION = 15.0;

static constexpr double IMAGE_ANIMATION_TIME =
    MILES_ICON_ANIMATION_TIME + MILES_ICON_ANIMATION_DURATION;
...
static constexpr double CONTAINER_LINE_ANIMATION_TIME =
    SCANLINES_ANIMATION_DURATION;
static constexpr double CONTAINER_LINE_ANIMATION_DURATION = 23.0;

static constexpr double CONTAINER_OUTER_TIME =
    SCANLINES_ANIMATION_DURATION + CONTAINER_LINE_ANIMATION_DURATION;
static constexpr double CONTAINER_OUTER_DURATION = 23.0;

static constexpr double CONTAINER_INNER_TIME =
    SCANLINES_ANIMATION_DURATION + CONTAINER_LINE_ANIMATION_DURATION + 8.0;
static constexpr double CONTAINER_INNER_DURATION = 15.0;

static constexpr double ALL_ANIMATIONS_FULL_DURATION =
...
public:
  bool OnSDLEvent(SDL_Event *event) override {
    ...
    if (event->type == SDL_QUIT && g_currentPage == WizardPage::Installing) {
      // Pretend the back button was pressed if the user tried quitting during
      // installation. This condition is above the rest of the event processing
      // as we want to block the exit button while there's confirmation message
      // is open as well.
      if (noModals)
    ...
    case SDL_KEYDOWN: {
      ...
      case SDL_SCANCODE_RIGHT:
        tapDirection.x =
            (event->key.keysym.scancode == SDL_SCANCODE_RIGHT) ? 1.0f : -1.0f;
        break;
      case SDL_SCANCODE_UP:
      case SDL_SCANCODE_DOWN:
        tapDirection.y =
            (event->key.keysym.scancode == SDL_SCANCODE_DOWN) ? 1.0f : -1.0f;
        break;
    ...
    case SDL_CONTROLLERAXISMOTION: {
      if (event->caxis.axis < 2) {
        ...
        bool sameDirection =
            (newAxisValue * g_joypadAxis[event->caxis.axis]) > 0.0f;
    ...
    case SDL_MOUSEMOTION: {
      for (size_t i = 0; i < g_currentCursorRects.size(); i++) {
        ...
        if (ImGui::IsMouseHoveringRect(currentRect.first, currentRect.second,
                                       false)) {
    ...
    if (tapDirection.x != 0.0f || tapDirection.y != 0.0f) {
      if (g_currentCursorIndex >= g_currentCursorRects.size() ||
          g_currentCursorIndex < 0) {
        newCursorIndex = g_currentCursorDefault;
      } else {
        auto &currentRect = g_currentCursorRects[g_currentCursorIndex];
        ImVec2 currentPoint =
            ImVec2((currentRect.first.x + currentRect.second.x) / 2.0f +
                       tapDirection.x *
                           (currentRect.second.x - currentRect.first.x) / 2.0f,
                   (currentRect.first.y + currentRect.second.y) / 2.0f +
                       tapDirection.y *
                           (currentRect.second.y - currentRect.first.y) / 2.0f);
        ...
        for (size_t i = 0; i < g_currentCursorRects.size(); i++) {
          ...
          ImVec2 targetPoint =
              ImVec2((targetRect.first.x + targetRect.second.x) / 2.0f +
                         tapDirection.x *
          ...
          ImVec2 delta = ImVec2(targetPoint.x - currentPoint.x,
                                targetPoint.y - currentPoint.y);
          float projectedDistance =
              delta.x * tapDirection.x + delta.y * tapDirection.y;
...
static std::string &GetWizardText(WizardPage page) {
  ...
  case WizardPage::SelectLanguage:
    return Localise("Installer_Page_SelectLanguage");
  case WizardPage::Introduction:
    return Localise("Installer_Page_Introduction");
  case WizardPage::SelectGameAndUpdate:
    return Localise("Installer_Page_SelectGameAndUpdate");
  case WizardPage::SelectDLC:
    return Localise("Installer_Page_SelectDLC");
  case WizardPage::CheckSpace:
    return Localise("Installer_Page_CheckSpace");
  case WizardPage::Installing:
    return Localise("Installer_Page_Installing");
  case WizardPage::InstallSucceeded:
    return Localise("Installer_Page_InstallSucceeded");
  case WizardPage::InstallFailed:
    return Localise("Installer_Page_InstallFailed");
  }
...
const char *DLC_SOURCE_TEXT[] = {
    "SPAGONIA", "CHUN-NAN",        "MAZURI",
    "HOLOSKA",  "APOTOS & SHAMAR", "EMPIRE CITY & ADABAT",
};
...
static double ComputeMotionInstaller(double timeAppear, double timeDisappear,
                                     double offset, double total) {
  return ComputeMotion(timeAppear, offset, total) *
         (1.0 - ComputeMotion(timeDisappear,
                              ALL_ANIMATIONS_FULL_DURATION - offset - total,
                              total));
}

static double ComputeMotionInstallerLoop(double timeAppear, double speed,
                                         double offset) {
  return std::clamp(
             fmodf((ImGui::GetTime() - timeAppear) * speed, 1.0f + offset) -
                 offset,
             0.0, 1.0) /
         1.0;
}

static double ComputeHermiteMotionInstallerLoop(double timeAppear, double speed,
                                                double offset) {
  return (cosf(M_PI * ComputeMotionInstallerLoop(timeAppear, speed, offset) +
               M_PI) +
          1) /
         2;
}

static bool PushCursorRect(ImVec2 min, ImVec2 max, bool &cursorPressed,
...
static void DrawLeftImage() {
  ...
  if (g_currentPage == WizardPage::Installing) {
    ...
    double installationTime =
        (ImGui::GetTime() - g_installerStartTime) * InstallationSpeed;
    installTextureIndex += int(installationTime);
  }

  double imageAlpha =
      ComputeMotionInstaller(g_appearTime, g_disappearTime,
                             IMAGE_ANIMATION_TIME, IMAGE_ANIMATION_DURATION);
  int a = std::lround(255.0 * imageAlpha);
  GuestTexture *guestTexture =
      g_installTextures[installTextureIndex % g_installTextures.size()].get();
  auto &res = ImGui::GetIO().DisplaySize;
  auto drawList = ImGui::GetBackgroundDrawList();
  ImVec2 min = {g_aspectRatioOffsetX + Scale(IMAGE_X),
                g_aspectRatioOffsetY + Scale(IMAGE_Y)};
  ImVec2 max = {min.x + Scale(IMAGE_WIDTH), min.y + Scale(IMAGE_HEIGHT)};
  drawList->AddImage(guestTexture, min, max, ImVec2(0, 0), ImVec2(1, 1),
                     IM_COL32(255, 255, 255, a));
}

static void DrawHeaderIconsForInstallPhase(double iconsPosX, double iconsPosY,
                                           double iconsScale) {
  auto drawList = ImGui::GetBackgroundDrawList();

  // Arrow Circle Icon
  ImVec2 arrowCircleMin = {g_aspectRatioOffsetX +
                               Scale(iconsPosX - iconsScale / 2),
                           Scale(iconsPosY - iconsScale / 2)};
  ImVec2 arrowCircleMax = {g_aspectRatioOffsetX +
                               Scale(iconsPosX + iconsScale / 2),
                           Scale(iconsPosY + iconsScale / 2)};
  ImVec2 center = {g_aspectRatioOffsetX + Scale(iconsPosX) + 0.5f,
                   Scale(iconsPosY) - 0.5f};

  float arrowCircleFadeMotion =
      ComputeMotionInstaller(g_installerStartTime, g_installerEndTime,
                             INSTALL_ICONS_FADE_IN_ANIMATION_TIME,
                             INSTALL_ICONS_FADE_IN_ANIMATION_DURATION);
  float rotationMotion = ComputeMotionInstallerLoop(g_installerStartTime,
                                                    ARROW_CIRCLE_LOOP_SPEED, 0);
  ...
  ImVec2 corners[4] = {
      ImRotate(ImVec2(arrowCircleMin.x - center.x, arrowCircleMin.y - center.y),
               cosCurrentAngle, sinCurrentAngle),
      ImRotate(ImVec2(arrowCircleMax.x - center.x, arrowCircleMin.y - center.y),
               cosCurrentAngle, sinCurrentAngle),
      ImRotate(ImVec2(arrowCircleMax.x - center.x, arrowCircleMax.y - center.y),
               cosCurrentAngle, sinCurrentAngle),
      ImRotate(ImVec2(arrowCircleMin.x - center.x, arrowCircleMax.y - center.y),
               cosCurrentAngle, sinCurrentAngle),
  };
  ...
  drawList->AddImageQuad(g_arrowCircle.get(), corners[0], corners[1],
                         corners[2], corners[3], ImVec2(0, 0), ImVec2(1, 0),
                         ImVec2(1, 1), ImVec2(0, 1),
                         IM_COL32(255, 255, 255, 96 * arrowCircleFadeMotion));

  // Pulse
  float pulseFadeMotion =
      ComputeMotionInstaller(g_installerStartTime, g_installerEndTime,
                             INSTALL_ICONS_FADE_IN_ANIMATION_TIME,
                             INSTALL_ICONS_FADE_IN_ANIMATION_DURATION);
  float pulseMotion = ComputeMotionInstallerLoop(g_installerStartTime,
                                                 PULSE_ANIMATION_LOOP_SPEED,
                                                 PULSE_ANIMATION_LOOP_DELAY);
  float pulseHermiteMotion = ComputeHermiteMotionInstallerLoop(
      g_installerStartTime, PULSE_ANIMATION_LOOP_SPEED,
      PULSE_ANIMATION_LOOP_DELAY);

  float pulseFade = pulseMotion / PULSE_ANIMATION_LOOP_FADE_HIGH_POINT;

  if (pulseMotion >= PULSE_ANIMATION_LOOP_FADE_HIGH_POINT) {
    // Calculate linear fade-out from high point time -
    // ({PULSE_ANIMATION_LOOP_FADE_HIGH_POINT}, 1) - to loop end - (1, 0) -.
  ...
  ImVec2 pulseMin = {g_aspectRatioOffsetX + Scale(iconsPosX - pulseScale / 2),
                     Scale(iconsPosY - pulseScale / 2)};
  ImVec2 pulseMax = {g_aspectRatioOffsetX + Scale(iconsPosX + pulseScale / 2),
                     Scale(iconsPosY + pulseScale / 2)};
  drawList->AddImage(
      g_pulseInstall.get(), pulseMin, pulseMax, ImVec2(0, 0), ImVec2(1, 1),
      IM_COL32(255, 255, 255, 255 * pulseFade * pulseFadeMotion));
}
...
static void DrawHeaderIcons() {
  ...
  // Miles Electric Icon
  float milesIconMotion = ComputeMotionInstaller(g_appearTime, g_disappearTime,
                                                 MILES_ICON_ANIMATION_TIME,
                                                 MILES_ICON_ANIMATION_DURATION);
  float milesIconScale = iconsScale * (2 - milesIconMotion);

  ImVec2 milesElectricMin = {g_aspectRatioOffsetX +
                                 Scale(iconsPosX - milesIconScale / 2),
                             Scale(iconsPosY - milesIconScale / 2)};
  ImVec2 milesElectricMax = {g_aspectRatioOffsetX +
                                 Scale(iconsPosX + milesIconScale / 2),
                             Scale(iconsPosY + milesIconScale / 2)};
  drawList->AddImage(g_milesElectricIcon.get(), milesElectricMin,
                     milesElectricMax, ImVec2(0, 0), ImVec2(1, 1),
                     IM_COL32(255, 255, 255, 255 * milesIconMotion));
...
static void DrawContainer(ImVec2 min, ImVec2 max, bool isTextArea) {
  ...
  double gridAlpha = ComputeMotionInstaller(
      g_appearTime, g_disappearTime,
      isTextArea ? CONTAINER_INNER_TIME : CONTAINER_OUTER_TIME,
      isTextArea ? CONTAINER_INNER_DURATION : CONTAINER_OUTER_DURATION);
  double gridOverlayAlpha =
      ComputeMotionInstaller(g_appearTime, g_disappearTime,
                             CONTAINER_INNER_TIME, CONTAINER_INNER_DURATION);

  const uint32_t gridColor =
      IM_COL32(0, 33, 0, (isTextArea ? 223 : 255) * gridAlpha);
  ...
  drawList->PushClipRect(
      {min.x - gridSize * 2.0f, min.y + gridSize * 2.0f},
...
static void DrawDescriptionContainer() {
  ...
  ImVec2 descriptionMin = {
      round(g_aspectRatioOffsetX + Scale(CONTAINER_X + 0.5f)),
      round(g_aspectRatioOffsetY + Scale(CONTAINER_Y + 0.5f))};
  ImVec2 descriptionMax = {
      round(g_aspectRatioOffsetX + Scale(CONTAINER_X + 0.5f + CONTAINER_WIDTH)),
      round(g_aspectRatioOffsetY +
            Scale(CONTAINER_Y + 0.5f + CONTAINER_HEIGHT))};
  SetProceduralOrigin(descriptionMin);
  DrawContainer(descriptionMin, descriptionMax, true);

  std::string descriptionText = GetWizardText(g_currentPage);
  ...
  if (g_currentPage == WizardPage::CheckSpace) {
    ...
    std::string requiredSpaceText = fmt::format(
        fmt::runtime(Localise("Installer_Step_RequiredSpace")), requiredGiB);
    std::string availableSpaceText =
        (g_installerAvailableSize > 0)
            ? fmt::format(
                  fmt::runtime(Localise("Installer_Step_AvailableSpace")),
                  availableGiB)
            : "";
    descriptionText = fmt::format("{}{}\n{}", GetWizardText(g_currentPage),
                                  requiredSpaceText, availableSpaceText);
    ...
    descriptionText += g_installerErrorMessage;
  }
  ...
  if (Config::Language == ELanguage::Japanese) {
    ...
    // Removing some padding of the applied due to the inclusion of annotation
    // for Japanese
    textX -= (fontSize + Scale(1.5f));
    textY -= Scale(7.0f);

    // The annotation (and thus the Japanese) can be drawn above the edges of
    // the info panel thus the clip needs to be extended a bit
  ...
  DrawRubyAnnotatedText(
      g_seuratFont, fontSize, lineWidth, {textX, textY}, lineMargin,
      descriptionText.c_str(),
      [=](const char *str, ImVec2 pos) {
        DrawTextBasic(g_seuratFont, fontSize, pos,
                      IM_COL32(255, 255, 255, 255 * textAlpha), str);
      },
      [=](const char *str, float size, ImVec2 pos) {
        DrawTextBasic(g_seuratFont, size, pos,
                      IM_COL32(255, 255, 255, 255 * textAlpha), str);
      },
  ...
  if (g_currentPage == WizardPage::InstallSucceeded) {
    auto descTextSize = MeasureCentredParagraph(
        g_seuratFont, fontSize, lineWidth, lineMargin, descriptionText);

    auto hedgeDevStr = "hedge-dev";
    auto hedgeDevTextSize =
        g_seuratFont->CalcTextSizeA(fontSize, FLT_MAX, 0, hedgeDevStr);
    ...
    auto marqueeTextSize =
        g_seuratFont->CalcTextSizeA(fontSize, FLT_MAX, 0, g_creditsStr.c_str());
    auto marqueeTextMarginX = Scale(5);
    auto marqueeTextMarginY = Scale(15);

    ImVec2 marqueeTextPos = {descriptionMax.x, containerBottom -
                                                   marqueeTextSize.y -
                                                   marqueeTextMarginY};
    ...
    ImVec2 imageRegionMax = {containerRight,
                             containerBottom -
                                 (marqueeTextMax.y - marqueeTextMin.y)};

    ImVec2 imageMin = {
        /* X */ imageRegionMin.x + ((imageRegionMax.x - imageRegionMin.x) / 2) -
            (imageScale / 2) - (hedgeDevTextSize.x / 2) - hedgeDevTextMarginX,
        /* Y */ imageRegionMin.y + ((imageRegionMax.y - imageRegionMin.y) / 2) -
            (imageScale / 2) - imageMarginY};

    ImVec2 imageMax = {imageMin.x + imageScale, imageMin.y + imageScale};

    drawList->AddImage(g_upHedgeDev.get(), imageMin, imageMax, {0, 0}, {1, 1},
                       colWhite);

    drawList->AddText(
        g_seuratFont, fontSize,
        {/* X */ imageMax.x + hedgeDevTextMarginX,
         /* Y */ imageMin.y + (imageScale / 2) - (hedgeDevTextSize.y / 2)},
        colWhite, hedgeDevStr);

    SetHorizontalMarqueeFade(marqueeTextMin, marqueeTextMax, Scale(32));
    DrawTextWithMarquee(g_seuratFont, fontSize, marqueeTextPos, marqueeTextMin,
                        marqueeTextMax, colWhite, g_creditsStr.c_str(),
                        g_installerEndTime, 0.9, Scale(200));
    ResetMarqueeFade();
  }
...
static void DrawButtonContainer(ImVec2 min, ImVec2 max, int baser, int baseg,
                                float alpha) {
  auto &res = ImGui::GetIO().DisplaySize;
  auto drawList = ImGui::GetBackgroundDrawList();
  SetShaderModifier(IMGUI_SHADER_MODIFIER_SCANLINE_BUTTON);
  drawList->AddRectFilledMultiColor(
      min, max, IM_COL32(baser, baseg + 130, 0, 223 * alpha),
      IM_COL32(baser, baseg + 130, 0, 178 * alpha),
      IM_COL32(baser, baseg + 130, 0, 223 * alpha),
      IM_COL32(baser, baseg + 130, 0, 178 * alpha));
  drawList->AddRectFilledMultiColor(
      min, max, IM_COL32(baser, baseg, 0, 13 * alpha),
      IM_COL32(baser, baseg, 0, 0), IM_COL32(baser, baseg, 0, 55 * alpha),
      IM_COL32(baser, baseg, 0, 6 * alpha));
  drawList->AddRectFilledMultiColor(
      min, max, IM_COL32(baser, baseg + 130, 0, 13 * alpha),
      IM_COL32(baser, baseg + 130, 0, 111 * alpha),
      IM_COL32(baser, baseg + 130, 0, 0),
      IM_COL32(baser, baseg + 130, 0, 55 * alpha));
  SetShaderModifier(IMGUI_SHADER_MODIFIER_NONE);
}

static ImVec2 ComputeTextSize(ImFont *font, const char *text, float size,
                              float &squashRatio,
...
static void DrawButton(ImVec2 min, ImVec2 max, const char *buttonText,
                       bool sourceButton, bool buttonEnabled,
                       bool &buttonPressed, float maxTextWidth = FLT_MAX,
                       bool makeDefault = false) {
  buttonPressed = false;
  ...
  int baser = 0;
  int baseg = 0;
  if (g_currentMessagePrompt.empty() && !g_currentPickerVisible &&
      !sourceButton && buttonEnabled && (alpha >= 1.0f)) {
  ...
  float squashRatio;
  ImVec2 textSize =
      ComputeTextSize(font, buttonText, size, squashRatio, Scale(maxTextWidth));
  ...
  SetGradient(min, {min.x + textSize.x, min.y + textSize.y},
              IM_COL32(baser + 192, 255, 0, 255),
              IM_COL32(baser + 128, baseg + 170, 0, 255));

  DrawTextWithOutline(font, size, min, IM_COL32(255, 255, 255, 255 * alpha),
                      buttonText, 4, IM_COL32(baser, baseg, 0, 255 * alpha));
...
static void ComputeButtonColumnCoordinates(ButtonColumn buttonColumn,
  ...
  case ButtonColumnMiddle:
    minX = g_aspectRatioOffsetX + Scale(CONTAINER_X + CONTAINER_WIDTH / 2.0f -
                                        CONTAINER_BUTTON_WIDTH / 2.0f);
    maxX = g_aspectRatioOffsetX + Scale(CONTAINER_X + CONTAINER_WIDTH / 2.0f +
                                        CONTAINER_BUTTON_WIDTH / 2.0f);
    break;
  case ButtonColumnRight:
    minX = g_aspectRatioOffsetX +
           Scale(CONTAINER_X + CONTAINER_WIDTH - CONTAINER_BUTTON_GAP -
                 CONTAINER_BUTTON_WIDTH);
    maxX = g_aspectRatioOffsetX +
           Scale(CONTAINER_X + CONTAINER_WIDTH - CONTAINER_BUTTON_GAP);
    break;
  }
...
static void DrawSourceButton(ButtonColumn buttonColumn, float yRatio,
                             const char *sourceText, bool sourceSet) {
  bool buttonPressed;
  float minX, maxX;
  ComputeButtonColumnCoordinates(buttonColumn, minX, maxX);

  float minusY = (CONTAINER_BUTTON_GAP + BUTTON_HEIGHT) * yRatio;
  ImVec2 min = {minX, g_aspectRatioOffsetY +
                          Scale(CONTAINER_Y + CONTAINER_HEIGHT -
                                CONTAINER_BUTTON_GAP - BUTTON_HEIGHT - minusY)};
  ImVec2 max = {maxX,
                g_aspectRatioOffsetY + Scale(CONTAINER_Y + CONTAINER_HEIGHT -
                                             CONTAINER_BUTTON_GAP - minusY)};
  ...
  DrawButton(min, max, sourceText, true, sourceSet, buttonPressed,
             ((max.x - min.x) * 0.7) / g_aspectRatioScale);
  DrawToggleLight(
      {min.x + lightSize, min.y + ((max.y - min.y) - lightSize) / 2 + Scale(1)},
      sourceSet, (sourceSet ? 1.0f : 0.5f) * alphaMotion);
}
...
static void DrawProgressBar(float progressRatio) {
  ...
  ImVec2 min = {g_aspectRatioOffsetX + Scale(CONTAINER_X) + BOTTOM_X_GAP +
                    Scale(1),
                g_aspectRatioOffsetY +
                    Scale(CONTAINER_Y + CONTAINER_HEIGHT + BOTTOM_Y_GAP)};
  ImVec2 max = {g_aspectRatioOffsetX +
                    Scale(CONTAINER_X + CONTAINER_WIDTH - BOTTOM_X_GAP),
                g_aspectRatioOffsetY + Scale(CONTAINER_Y + CONTAINER_HEIGHT +
                                             BOTTOM_Y_GAP + BUTTON_HEIGHT)};

  DrawButtonContainer(min, max, 0, 0, alpha);

  drawList->AddRectFilledMultiColor({min.x + xPadding, min.y + yPadding},
                                    {max.x - xPadding, max.y - yPadding},
                                    innerColor0, innerColor0, innerColor1,
                                    innerColor1);
  ...
  drawList->AddRectFilledMultiColor(sliderMin, sliderMax, sliderColor0,
                                    sliderColor0, sliderColor1, sliderColor1);
}

static bool ConvertPathSet(const nfdpathset_t *pathSet,
...
static void PickerStart(bool folderMode) {
  ...
  // Optional single thread mode for testing on systems that do not interact
  // well with the separate thread being used for NFD.
#ifdef __APPLE__
...
static void PickerShow(bool folderMode) {
  if (g_pickerTutorialCleared[folderMode]) {
    ...
    g_currentMessagePrompt =
        Localise(folderMode ? "Installer_Message_FolderPickerTutorial"
...
static bool ParseSourcePaths(std::list<std::filesystem::path> &paths) {
  assert((g_currentPage == WizardPage::SelectGameAndUpdate) ||
  ...
  if (!failedPaths.empty()) {
    ...
    for (const std::filesystem::path &path : failedPaths) {
      ...
      stringStream << std::endl
                   << "- "
                   << Truncate(
                          std::string(filenameU8.begin(), filenameU8.end()), 32,
                          true, true);
    }
...
static void DrawLanguagePicker() {
  ...
  if (g_currentPage == WizardPage::SelectLanguage) {
    ...
    for (int i = 0; i < 6; i++) {
      ComputeButtonColumnCoordinates(
          (i < 3) ? ButtonColumnLeft : ButtonColumnRight, minX, maxX);
      ...
      ImVec2 min = {minX,
                    g_aspectRatioOffsetY +
                        Scale(CONTAINER_Y + CONTAINER_HEIGHT -
                              CONTAINER_BUTTON_GAP - BUTTON_HEIGHT - minusY)};
      ImVec2 max = {maxX, g_aspectRatioOffsetY +
                              Scale(CONTAINER_Y + CONTAINER_HEIGHT -
                                    CONTAINER_BUTTON_GAP - minusY)};
      ...
      DrawButton(min, max, LANGUAGE_TEXT[i], false, true, buttonPressed,
                 FLT_MAX, LANGUAGE_ENUM[i] == ELanguage::English);
      DrawToggleLight({min.x + lightSize,
                       min.y + ((max.y - min.y) - lightSize) / 2 + Scale(1)},
...
static void DrawSourcePickers() {
  ...
  if (g_currentPage == WizardPage::SelectGameAndUpdate ||
    ...
    float squashRatio;
    ImVec2 textSize =
        ComputeTextSize(g_dfsogeistdFont, addFilesText.c_str(), 20.0f,
                        squashRatio, ADD_BUTTON_MAX_TEXT_WIDTH);
    ImVec2 min = {g_aspectRatioOffsetX + Scale(CONTAINER_X + BOTTOM_X_GAP),
                  g_aspectRatioOffsetY +
                      Scale(CONTAINER_Y + CONTAINER_HEIGHT + BOTTOM_Y_GAP)};
    ImVec2 max = {g_aspectRatioOffsetX +
                      Scale(CONTAINER_X + BOTTOM_X_GAP +
                            textSize.x * squashRatio + BUTTON_TEXT_GAP),
                  g_aspectRatioOffsetY + Scale(CONTAINER_Y + CONTAINER_HEIGHT +
                                               BOTTOM_Y_GAP + BUTTON_HEIGHT)};
    DrawButton(min, max, addFilesText.c_str(), false, true, buttonPressed,
    ...
    DrawButton(min, max, addFolderText.c_str(), false, true, buttonPressed,
               ADD_BUTTON_MAX_TEXT_WIDTH);
    if (buttonPressed) {
      PickerShow(true);
    }
...
static void DrawSources() {
  if (g_currentPage == WizardPage::SelectGameAndUpdate) {
    DrawSourceButton(ButtonColumnLeft, 0,
                     Localise("Installer_Step_Game").c_str(),
                     !g_gameSourcePath.empty());
    DrawSourceButton(ButtonColumnRight, 0,
                     Localise("Installer_Step_Update").c_str(),
                     !g_updateSourcePath.empty());
  }

  if (g_currentPage == WizardPage::SelectDLC) {
    for (int i = 0; i < 6; i++) {
      DrawSourceButton((i < 3) ? ButtonColumnLeft : ButtonColumnRight,
                       float(i % 3), DLC_SOURCE_TEXT[i],
                       !g_dlcSourcePaths[i].empty() || g_dlcInstalled[i]);
    }
  }
}
...
static void DrawInstallingProgress() {
  if (g_currentPage == WizardPage::Installing) {
    ...
    g_installerProgressRatioCurrent +=
        std::min(ratioTarget - g_installerProgressRatioCurrent,
    ...
    if (g_installerFinished) {
      ...
      g_currentPage = g_installerFailed ? WizardPage::InstallFailed
                                        : WizardPage::InstallSucceeded;
    }
  }
}

static void InstallerThread() {
  if (!Installer::install(g_installerSources, g_installPath, false,
                          g_installerJournal, std::chrono::seconds(1), [&]() {
                            g_installerProgressRatioTarget = float(
                                double(g_installerJournal.progressCounter) /
                                double(g_installerJournal.progressTotal));

                            // If user is being asked for confirmation on
                            // cancelling the installation, halt the installer
                            // from progressing further.
                            g_installerHalted.wait(true);

                            // If user has confirmed they wish to cancel the
                            // installation, return false to indicate the
                            // installer should fail and stop.
...
static bool InstallerParseSources(std::string &errorMessage) {
  ...
  std::filesystem::space_info spaceInfo =
      std::filesystem::space(g_installPath, spaceErrorCode);
  if (!spaceErrorCode) {
  ...
  bool sourcesParsed = Installer::parseSources(
      installerInput, g_installerJournal, g_installerSources);
...
static void DrawNavigationButton() {
  ...
  bool nextButtonEnabled =
      !g_isDisappearing && (g_currentPage != WizardPage::Installing);
  if (nextButtonEnabled && g_currentPage == WizardPage::SelectGameAndUpdate) {
    nextButtonEnabled =
        !g_gameSourcePath.empty() && !g_updateSourcePath.empty();
  }

  bool skipButton = false;
  if (g_currentPage == WizardPage::SelectDLC) {
    skipButton = std::all_of(
        g_dlcSourcePaths.begin(), g_dlcSourcePaths.end(),
  ...
  ImVec2 nextTextSize =
      ComputeTextSize(g_newRodinFont, nextButtonText.c_str(), 20.0f,
                      squashRatio, NAV_BUTTON_MAX_TEXT_WIDTH);
  ImVec2 min = {g_aspectRatioOffsetX + Scale(CONTAINER_X + CONTAINER_WIDTH -
                                             nextTextSize.x * squashRatio -
                                             BOTTOM_X_GAP - BUTTON_TEXT_GAP),
                g_aspectRatioOffsetY +
                    Scale(CONTAINER_Y + CONTAINER_HEIGHT + BOTTOM_Y_GAP)};
  ImVec2 max = {g_aspectRatioOffsetX +
                    Scale(CONTAINER_X + CONTAINER_WIDTH - BOTTOM_X_GAP),
  ...
  bool buttonPressed = false;
  DrawButton(min, max, nextButtonText.c_str(), false, nextButtonEnabled,
             buttonPressed, NAV_BUTTON_MAX_TEXT_WIDTH);

  if (buttonPressed) {
    XexPatcher::Result patcherResult;
    if (g_currentPage == WizardPage::SelectGameAndUpdate &&
        (patcherResult = Installer::checkGameUpdateCompatibility(
             g_gameSourcePath, g_updateSourcePath),
         patcherResult != XexPatcher::Result::Success)) {
      g_currentMessagePrompt =
          Localise("Installer_Message_IncompatibleGameData");
      g_currentMessagePromptConfirmation = false;
    } else if (g_currentPage == WizardPage::SelectDLC) {
      ...
      if (!InstallerParseSources(sourcesErrorMessage)) {
        // Some of the sources that were provided to the installer are not
        // valid. Restart the file selection process.
        std::stringstream stringStream;
        ...
        g_currentMessagePrompt = stringStream.str();
        g_currentMessagePromptConfirmation = false;
        g_currentPage = dlcInstallerMode ? WizardPage::SelectDLC
                                         : WizardPage::SelectGameAndUpdate;
      } else if (dlcIncomplete && !dlcInstallerMode) {
        // Not all the DLC was specified, we show a prompt and await a
        // confirmation before starting the installer.
        ...
        // Nothing was selected and the installer was in DLC mode, just close
        // it.
...
static void CheckCancelAction() {
  ...
  if (g_currentPage == g_firstPage ||
    ...
    // Indicate to the installer that all progress should stop until the user
    // confirms if they wish to cancel.
...
static void DrawHorizontalBorder(bool bottomBorder) {
  ...
  auto drawList = ImGui::GetBackgroundDrawList();
  double borderScale =
      1.0 - ComputeMotionInstaller(g_appearTime, g_disappearTime,
                                   CONTAINER_LINE_ANIMATION_TIME,
                                   CONTAINER_LINE_ANIMATION_DURATION);
  float midX = g_aspectRatioOffsetX + Scale(CONTAINER_X + CONTAINER_WIDTH / 5);
  float minX =
      std::lerp(g_aspectRatioOffsetX +
                    Scale(CONTAINER_X - BORDER_SIZE - BORDER_OVERSHOOT),
                midX, borderScale);
  float maxX = std::lerp(g_aspectRatioOffsetX +
                             Scale(CONTAINER_X + CONTAINER_WIDTH +
                                   SIDE_CONTAINER_WIDTH + BORDER_OVERSHOOT),
                         midX, borderScale);
  float minY = g_aspectRatioOffsetY +
               (bottomBorder ? Scale(CONTAINER_Y + CONTAINER_HEIGHT)
                             : Scale(CONTAINER_Y - BORDER_SIZE));
  float maxY = minY + Scale(BORDER_SIZE);
  drawList->AddRectFilledMultiColor({minX, minY}, {midX, maxY}, FADE_COLOR_LEFT,
                                    SOLID_COLOR, SOLID_COLOR, FADE_COLOR_LEFT);

  drawList->AddRectFilledMultiColor({midX, minY}, {maxX, maxY}, SOLID_COLOR,
                                    FADE_COLOR_RIGHT, FADE_COLOR_RIGHT,
                                    SOLID_COLOR);
}
...
static void DrawVerticalBorder(bool rightBorder) {
  ...
  double borderScale =
      1.0 - ComputeMotionInstaller(g_appearTime, g_disappearTime,
                                   ...
                                   CONTAINER_LINE_ANIMATION_DURATION);
  float minX =
      g_aspectRatioOffsetX + (rightBorder ? Scale(CONTAINER_X + CONTAINER_WIDTH)
                                          : Scale(CONTAINER_X - BORDER_SIZE));
  float maxX = minX + Scale(BORDER_SIZE);
  float midY = g_aspectRatioOffsetY + Scale(CONTAINER_Y + CONTAINER_HEIGHT / 2);
  float minY =
      std::lerp(g_aspectRatioOffsetY + Scale(CONTAINER_Y - BORDER_OVERSHOOT),
                midY, borderScale);
  float maxY =
      std::lerp(g_aspectRatioOffsetY +
                    Scale(CONTAINER_Y + CONTAINER_HEIGHT + BORDER_OVERSHOOT),
                midY, borderScale);
  drawList->AddRectFilledMultiColor({minX, minY}, {maxX, midY}, FADE_COLOR,
                                    FADE_COLOR, SOLID_COLOR, SOLID_COLOR);

  drawList->AddRectFilledMultiColor({minX, midY}, {maxX, maxY}, SOLID_COLOR,
                                    SOLID_COLOR, FADE_COLOR, FADE_COLOR);
}
...
static void DrawMessagePrompt() {
  ...
  if (g_currentMessagePromptConfirmation) {
    std::array<std::string, 2> YesNoButtons = {Localise("Common_Yes"),
                                               Localise("Common_No")};
    messageWindowReturned = MessageWindow::Open(
        g_currentMessagePrompt, &g_currentMessageResult, YesNoButtons, 1);
  } else {
    messageWindowReturned =
        MessageWindow::Open(g_currentMessagePrompt, &g_currentMessageResult);
  }
  ...
  if (messageWindowReturned) {
    if (g_currentMessagePromptConfirmation && (g_currentMessageResult == 0)) {
      if (g_currentMessagePromptSource == MessagePromptSource::Back) {
        if (g_currentPage == WizardPage::Installing) {
          // If user confirms they wish to cancel the installation, notify the
          // installation thread it must finish as soon as possible.
        ...
        // If user confirms the message prompt that they wish to skip installing
        // the DLC, proceed to the next step.
        g_currentPage = WizardPage::CheckSpace;
      }
    }

    if (g_currentMessagePromptSource == MessagePromptSource::Back) {
      // Regardless of the confirmation, the installation thread must be
      // resumed.
...
static void PickerCheckResults() {
  ...
  if (!g_currentPickerResults.empty() &&
...
void InstallerWizard::Draw() {
  ...
  double scanlinesAlpha = ComputeMotionInstaller(
      g_appearTime, g_disappearTime, 0.0, SCANLINES_ANIMATION_DURATION);
  float height = Scale(105.0f) * (float)scanlinesAlpha;

  DrawScanlineBars(height, (float)scanlinesAlpha, [&]() {
    auto &headerText = Localise(g_currentPage == WizardPage::Installing
                                    ? "Installer_Header_Installing"
                                    : "Installer_Header_Installer");
    auto alphaMotion =
        ComputeMotionInstaller(g_appearTime, g_disappearTime,
                               TITLE_ANIMATION_TIME, TITLE_ANIMATION_DURATION);
...
bool InstallerWizard::Run(std::filesystem::path installPath, bool skipGame) {
  ...
  // Guarantee one controller is initialized. We'll rely on SDL's event loop to
